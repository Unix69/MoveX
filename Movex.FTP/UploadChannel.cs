...
namespace Movex.FTP
{
    public class UploadChannel
    {
        ...
        //creater
        public static UploadChannel JoinUploadChannels(UploadChannel uchan1, UploadChannel uchan2) {
            var uchan = new UploadChannel();
            string[] filenamesuchan1;
            string[] filenamesuchan2;
            int[] filesizesuchan1;
            int[] filesizesuchan2;
            string[] filepathsuchan1;
            string[] filepathsuchan2;
            string[] filepathsuchan;
            string[] filenamesuchan;
            int [] filesizesuchan;
            var k = 0;
            var flag = 0;

            if (!uchan1.Get_to().Equals(uchan2.Get_to())) {
                return (null);
            }

            filenamesuchan1 = uchan1.Get_filenames();
            filenamesuchan2 = uchan2.Get_filenames();

            filesizesuchan1 = uchan1.Get_filesizes();
            filesizesuchan2 = uchan2.Get_filesizes();

            filepathsuchan1 = uchan1.Get_paths();
            filepathsuchan2 = uchan2.Get_paths();

            filenamesuchan = new string[filenamesuchan1.Length + filenamesuchan2.Length];
            filepathsuchan = new string[filepathsuchan1.Length + filepathsuchan2.Length];
            filesizesuchan = new int[filesizesuchan1.Length + filesizesuchan2.Length];

            filenamesuchan1.CopyTo(filenamesuchan, 0);
            filepathsuchan1.CopyTo(filepathsuchan, 0);
            filesizesuchan1.CopyTo(filesizesuchan, 0);

            k = filenamesuchan1.Length;
            var bitmap = new int[k];

            for (var i = 0; i < filenamesuchan2.Length; i++) {

                for (var j = 0; j < filenamesuchan1.Length; j++){
                    if (bitmap[j] != 1 && filenamesuchan1[i].Equals(filenamesuchan2[j]) && filepathsuchan1[i].Equals(filepathsuchan2[j])) {
                        flag = 1;
                        bitmap[j] = 1;
                        break;
                    }
                ...
                if (flag != 1)
                {
                    filenamesuchan[k] = filenamesuchan2[i];
                    filepathsuchan[k] = filepathsuchan2[i];
                    filesizesuchan[k] = filesizesuchan2[i];
                    k++;
                }
                else {
                    flag = 0;
            ...
            uchan.Set_num_trasf(k);

            Array.Resize(ref filenamesuchan, k);
            Array.Resize(ref filepathsuchan, k);
            Array.Resize(ref filesizesuchan, k);

            uchan.Set_paths(filepathsuchan);
            uchan.Set_filenames(filenamesuchan);
            uchan.Set_filesizes(filesizesuchan);
